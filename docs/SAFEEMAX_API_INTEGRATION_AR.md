...
ثم استخدم `"deployment_mode": "ACTIVE"`. يمنع loader تفعيل الإرسال في
`SHADOW` التزاماً بعقد المشروع، كما يمنع الإرسال عند تشغيل replay حتى لا تُنشأ
تنبيهات تجريبية في واجهة الشركة.

تنبيه: ملف `models/drowsiness/active_model.json` الحالي يعرّف النموذج على أنه
`EXPERIMENTAL` و`SHADOW`، ولذلك لا يكفي تغيير ملف runtime وحده إلى `ACTIVE`؛ يجب
أولاً اعتماد bundle مسموح له بالعمل في `ACTIVE` وتفعيله وفق سياسة نشر النموذج.
